...
class SimpleStore {
  ...
  get filePath() {
    if (!this._filePath) {
      this._filePath = path.join(electron.app.getPath("userData"), `${this.name}.json`);
    }
  ...
  get(key2) {
    return key2.split(".").reduce((obj, k) => {
...
class Connector {
  constructor(context, captchaHandler) {
    __publicField(this, "context");
    __publicField(this, "captchaHandler");
  ...
  async fastType(page, selector, text) {
    ...
    await input.waitFor({ state: "visible", timeout: 5e3 });
    await input.click();
   ...
   */
  async ensureLoggedIn(page, creds) {
...
class BinLiConnector extends Connector {
  constructor() {
    super(...arguments);
    __publicField(this, "platformId", "binli");
    __publicField(this, "platformName", "彬利");
    __publicField(this, "baseUrl", "https://www.twbingli.com/index.php");
  }
  ...
  async search(page, searchTerm, filters) {
    ...
    if (filters == null ? void 0 : filters.code) {
      targetSelector = 'input[name="hid"]';
    } else if (filters == null ? void 0 : filters.component) {
    ...
    while (pageCount <= MAX_PAGES) {
      try {
        await page.waitForSelector(".name-ingredient", { timeout: 8e3 });
        await page.waitForTimeout(800);
      } catch (e) {
        console.log(`[BinLi] 第 ${pageCount} 頁無資料或載入超時`);
        break;
      }
      const pageResults = await page.evaluate((platform) => {
        ...
        nameElements.forEach((nameEl) => {
          ...
          const nhiCodeEl = row.querySelector(".nhi-code");
          const nhiCode = (nhiCodeEl == null ? void 0 : nhiCodeEl.innerText.trim()) || "";
          const nhiPriceEl = row.querySelector(".nhi .red");
          ...
          const stockText = (stockEl == null ? void 0 : stockEl.innerText.split("\n")[0].trim()) || "有供貨";
          const ingredientsEl = row.querySelector(".ingredients");
          const spec = (ingredientsEl == null ? void 0 : ingredientsEl.innerText.trim()) || "";
          if (name && name !== "品名(中英)") {
            products.push({
              platform,
              name,
      ...
      const isLastPage = await page.evaluate(() => {
        ...
        return (nextLi == null ? void 0 : nextLi.classList.contains("disabled")) || false;
      });
      if (isLastPage) {
...
class ChahwaConnector extends Connector {
  constructor(context) {
    super(context);
    __publicField(this, "platformId", "chahwa");
    __publicField(this, "platformName", "嘉鏵");
    __publicField(this, "baseUrl", "https://www.chahwa.com.tw/user.php");
    __publicField(this, "context");
    __publicField(this, "savedCreds", null);
    this.context = context;
  }
  /**
  ...
  async search(page, searchTerm, filters) {
    ...
    const performSearchStep = async () => {
      ...
      const isCode = (filters == null ? void 0 : filters.code) === true;
      const isComponent = (filters == null ? void 0 : filters.component) === true;
      let targetSelector = 'input[name="drug"]';
      if (isCode) {
        targetSelector = 'input[name="hid"]';
      } else if (isComponent) {
    ...
    try {
      while (pageCount <= MAX_PAGES) {
        ...
        if (this.lastPageSignature === currentPageSignature) {
        ...
        await page.waitForTimeout(2e3);
        try {
          await page.waitForSelector(".item", { timeout: 5e3 });
        } catch (e) {
...
class JhaoHongConnector extends Connector {
  constructor() {
    super(...arguments);
    __publicField(this, "platformId", "jhao-hong");
    __publicField(this, "platformName", "兆宇 (兆宏)");
    __publicField(this, "baseUrl", "https://www.jhao-hong.com.tw/");
  }
  async isLoggedIn(page) {
    try {
      const currentUrl = page.url();
      const logoutBtn = await page.$('button#logout, a:has-text("登出"), .user-info');
      const isLoggedIn = logoutBtn !== null || currentUrl.includes("prdindex.php") || currentUrl.includes("order.php");
  ...
  async search(page, searchTerm, filters) {
    ...
    try {
      ...
      if (!input) {
        await page.waitForSelector("input.search", { timeout: 3e3 });
        input = await page.$("input.search");
      }
      if (input) {
    ...
    while (pageCount <= MAX_PAGES) {
      try {
        await page.waitForSelector("li.item", { timeout: 8e3 });
        await page.waitForTimeout(500);
      } catch (e) {
      ...
      const pageResults = await page.evaluate((platform) => {
        try {
          ...
          return cards.map((card) => {
            ...
            const stockStatus = stockText.includes("缺貨") || stockText.includes("售完") || card.classList.contains("out") ? "缺貨中" : stockText || "有供貨";
      ...
      const isLastPage = await page.evaluate(() => {
        var _a;
        return ((_a = document.querySelector("li.r-arrow")) == null ? void 0 : _a.classList.contains("dis")) || false;
      });
      if (isLastPage) {
...
class YesChainConnector extends Connector {
  ...
  async login(page, creds) {
    ...
    try {
      ...
      await emailInput.waitFor({ state: "visible", timeout: 8e3 });
      ...
      if (await rcode.isVisible()) {
        ...
        if (this.captchaHandler) {
          try {
            ...
            await captchaImg.waitFor({ state: "visible", timeout: 1e4 });
            await page.waitForTimeout(500);
  ...
  async search(page, searchTerm, filters) {
    try {
      ...
      if ((filters == null ? void 0 : filters.code) === true) {
        searchInput = 'input[placeholder*="健保碼"]';
      } else if ((filters == null ? void 0 : filters.component) === true) {
        searchInput = 'input[placeholder*="成分"]';
      }
      await page.waitForSelector(searchInput, { state: "visible", timeout: 1e4 });
    ...
    while (pageCount <= MAX_PAGES) {
      try {
        await page.waitForSelector("tr", { timeout: 8e3 });
        await page.waitForTimeout(500);
      } catch (e) {
      ...
      const hasNextPage = await page.evaluate(() => {
        return Array.from(document.querySelectorAll("div.justify-end span, span")).some(
          (s) => {
            var _a;
            return ((_a = s.innerText) == null ? void 0 : _a.trim()) === "下一頁";
          }
        );
      });
      if (!hasNextPage) {
      ...
      const firstRowText = await page.evaluate(() => {
        ...
        return ((_a = rows[0]) == null ? void 0 : _a.innerText) || "";
      });
      await page.evaluate(() => {
...
class YuShengConnector extends Connector {
  constructor(context) {
    super(context);
    __publicField(this, "platformId", "yusheng");
    __publicField(this, "platformName", "宇盛");
    __publicField(this, "baseUrl", "https://www.yusheng0307.com/");
    __publicField(this, "context");
    this.context = context;
  }
  async isLoggedIn(page) {
    try {
      ...
      const isLoggedIn = (bodyText == null ? void 0 : bodyText.includes("會員登出")) || false;
  ...
  async login(page, creds) {
    ...
    try {
      ...
      await loginTrigger.waitFor({ state: "visible", timeout: 5e3 });
      ...
      await accountInput.waitFor({ state: "visible", timeout: 5e3 });
  ...
  async search(page, searchTerm, filters) {
    ...
    while (pageCount <= MAX_PAGES) {
      try {
        await page.waitForSelector("li.d-lg-block", { timeout: 8e3 });
        await page.waitForTimeout(500);
      } catch (e) {
...
class MDTConnector extends Connector {
  constructor() {
    super(...arguments);
    __publicField(this, "platformId", "mdt");
    __publicField(this, "platformName", "蔓達特");
    __publicField(this, "baseUrl", "https://www.mdtky.com.tw/Member/login");
  }
  async isLoggedIn(page) {
    try {
      const currentUrl = page.url();
  ...
  async login(page, creds) {
    ...
    if (this.captchaHandler) {
      try {
        ...
        await captchaImg.waitFor({ state: "visible", timeout: 5e3 });
        const screenshot = await captchaImg.screenshot({ type: "png" });
        const base64Image = `data:image/png;base64,${screenshot.toString("base64")}`;
  ...
  async search(page, searchTerm, filters) {
    ...
    try {
      ...
      while (pageCount <= 50) {
        ...
        await page.waitForSelector(".productBox, li.pdConList, table tr", { timeout: 1e4 });
        await page.waitForTimeout(2e3);
        const pageResults = await page.evaluate((platform) => {
          ...
          if (table && headers.some((h) => h.includes("品名") || h.includes("代碼"))) {
            ...
            return rows.map((row) => {
              ...
              const nameText = ((_a = cells[nameIdx >= 0 ? nameIdx : 2]) == null ? void 0 : _a.innerText.trim()) || "";
              const [name, spec] = nameText.split("\n");
              const nhiCode = ((_b = cells[nhiIdx >= 0 ? nhiIdx : 3]) == null ? void 0 : _b.innerText.trim()) || "";
              const nhiPrice = parseFloat(((_c = cells[nhiPriceIdx >= 0 ? nhiPriceIdx : 4]) == null ? void 0 : _c.innerText.replace(/[^0-9.]/g, "")) || "0");
              const price = parseFloat(((_d = cells[priceIdx >= 0 ? priceIdx : 5]) == null ? void 0 : _d.innerText.replace(/[^0-9.]/g, "")) || "0");
              const expiry = ((_e = cells[expiryIdx >= 0 ? expiryIdx : 6]) == null ? void 0 : _e.innerText.trim()) || "";
              return {
                platform,
                name: name || "未知",
                spec: spec || "",
          ...
          return cards.map((card) => {
            ...
            const nhiCode = ((_a = nhiLine.match(/[A-Z][A-Z0-9][0-9]{8}/)) == null ? void 0 : _a[0]) || "";
            ...
            const expiry = ((_b = expiryLine.split(":").pop()) == null ? void 0 : _b.trim()) || "";
            return {
              platform,
              name: name || "未知",
              spec,
              price,
              unit: "單位",
              stock: card.innerText.includes("售完") ? "售完" : "有供貨",
              link: window.location.href,
              expiry,
              nhiCode,
              nhiPrice
...
class CodaConnector extends Connector {
  constructor(context) {
    super(context);
    __publicField(this, "platformId", "coda");
    __publicField(this, "platformName", "可達");
    __publicField(this, "baseUrl", "https://www.codadrug.com.tw/");
    __publicField(this, "context");
    this.context = context;
  }
  // ── Session check ──────────────────────────────────────────────
  ...
  async search(page, searchTerm, filters) {
    ...
    try {
      ...
      await this.fastType(page, searchInput, searchTerm);
      await page.keyboard.press("Enter");
      await page.waitForSelector("a.item", { timeout: 1e4 });
    } catch (e) {
...
class YCConnector extends Connector {
  constructor() {
    super(...arguments);
    __publicField(this, "platformId", "yc");
    __publicField(this, "platformName", "益全生技");
    __publicField(this, "baseUrl", "https://ycmedicine.com.tw");
  }
  async isLoggedIn(page) {
  ...
  async login(page, creds) {
    ...
    await page.waitForSelector(userField, { timeout: 1e4 });
...
class TaichungConnector extends Connector {
  constructor() {
    super(...arguments);
    __publicField(this, "platformId", "taichung");
    __publicField(this, "platformName", "泰昌藥品");
    __publicField(this, "baseUrl", "https://taichung-pc.com.tw/index.php");
  }
  async isLoggedIn(page) {
...
function requireEncodings() {
  ...
  (function(exports$1) {
    ...
    for (var i = 0; i < modules.length; i++) {
      ...
      mergeModules2(exports$1, module2);
    }
  })(encodings);
  return encodings;
}
...
(function(module2) {
  ...
  module2.exports.getCodec = function getCodec(encoding) {
    if (!module2.exports.encodings) {
      ...
      mergeModules2(module2.exports.encodings, raw);
    }
...
function normalizeName(s) {
  ...
  text = text.split(/[\s,，]/)[0];
  return text.replace(/[()（）\[\]]/g, "").replace(/[""〝〞]/g, "").replace(/毫克|公絲|毫升/g, "mg").replace(/微克/g, "mcg").trim();
}
...
const store = new SimpleStore("drug-price-compare");
...
class AutomationManager {
  ...
  async getPageForPlatform(platformId) {
...
electron.ipcMain.handle("get-credentials", (_event, platformId) => {
  const creds = store.get(`creds.${platformId}`);
  if (creds && creds.password) {
    try {
      const decrypted = electron.safeStorage.decryptString(Buffer.from(creds.password, "base64"));
...
electron.ipcMain.handle("save-credentials", (_event, { platformId, username, password }) => {
  const encrypted = electron.safeStorage.encryptString(password).toString("base64");
  store.set(`creds.${platformId}`, { username, password: encrypted });
  return { success: true };
});
electron.ipcMain.handle("perform-search", async (_event, { searchTerm, platforms, filters }) => {
  return await performSearch(searchTerm, platforms, filters);
});
const handleRequestCaptcha = async (platformId, platformName, image) => {
  return new Promise((resolve) => {
    ...
    win == null ? void 0 : win.webContents.send("request-captcha", { platformId, platformName, image });
  });
};
electron.ipcMain.handle("submit-captcha", async (_event, { platformId, code }) => {
...
async function performSearch(searchTerm, platforms, filters) {
  console.log(`[Main] Starting concurrent search for: "${searchTerm}", Filters: ${JSON.stringify(filters)}`);
  pendingCaptchas = [];
  win == null ? void 0 : win.webContents.send("clear-captchas");
  ...
  const searchTasks = connectors.map(async (connector) => {
    try {
      ...
      const creds = store.get(`creds.${connector.platformId}`);
      if (!creds) return [];
      const page = await automationManager.getPageForPlatform(connector.platformId);
      const decryptedPassword = electron.safeStorage.decryptString(Buffer.from(creds.password, "base64"));
      const success = await connector.ensureLoggedIn(page, { ...creds, password: decryptedPassword });
      win == null ? void 0 : win.webContents.send("clear-captcha-for-platform", connector.platformId);
      if (isSearchInterrupted) return [];
      if (success) {
  ...
  if (results.length > 0) {
    const minPrice = Math.min(...results.filter((r) => r.price > 0).map((r) => r.price));
    results.forEach((r) => {
      if (r.price === minPrice && r.price > 0) r.isCheapest = true;
    });
  }
  return results;
}
async function processNhiTxt(event, filePaths) {
  ...
  const appearanceMap = /* @__PURE__ */ new Map();
  const apNameMap = /* @__PURE__ */ new Map();
  ...
  for (const filePath of filePaths) {
    ...
    for await (const lineText of rl) {
      ...
      try {
        ...
        const dateEnd = lineBuf.slice(45, 53).toString().trim();
        const nameEn = decode(54, 250);
        let nameCnFull = "";
        let ingredients = [];
        let manufacturer = "";
        let license = "";
        if (isMaster) {
          ...
          license = decode(484, 550).split("\0")[0];
        ...
        if (price > 0 || isSpecialProject) {
          ...
          if (isNewer) {
            let combinedIng = ingredients.map((i) => cleanClinicalString(i)).filter((v, i, a) => v && a.indexOf(v) === i).join(" + ");
            ...
            const entry = {
              ...
              ing: combinedIng,
              ds: ((_a = (nameEn.match(/(\d+\.?\d*\s?(MG|ML|%|GM|IU))/i) || [])[0]) == null ? void 0 : _a.toUpperCase()) || "",
...
electron.ipcMain.handle("process-nhi-txt", (event, filePaths) => processNhiTxt(event, filePaths));
electron.ipcMain.handle("open-file-dialog", async (event) => {
...
electron.ipcMain.handle("auto-index-nhi", async (event) => {
...
electron.ipcMain.handle("search-nhi-local", async (_, { searchTerm, filters }) => {
  return await searchNhiLocal(searchTerm, filters);
});
async function searchNhiLocal(searchTerm, filters) {
  ...
  let filtered = nhiDatabase.filter((item) => {
  ...
  if (filtered.length === 0) {
    filtered = nhiDatabase.filter((item) => {
